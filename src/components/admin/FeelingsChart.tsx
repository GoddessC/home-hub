...
export const FeelingsChart = () => {
  ...
  const DayComponent = ({ day, ...buttonProps }: any) => {
    // DayPicker passes CalendarDay with a .date field
    const dateObj: Date | undefined = day?.date instanceof Date ? day.date : (day instanceof Date ? day : undefined);
    ...
    return (
        <Popover>
          <PopoverTrigger asChild>
            <button
              {...buttonProps}
              className={cn("h-9 w-9 p-0 font-normal relative flex items-center justify-center cursor-pointer", buttonProps?.className)}
            >
